...
void StartARKitCalibration(void (*const onFinish)(void)) {
	...
	for(ARVideoFormat *const format in ARWorldTrackingConfiguration.supportedVideoFormats) {
		if(format.captureDevicePosition == AVCaptureDevicePositionFront || format.framesPerSecond < config.videoFormat.framesPerSecond)
			continue;
		if((double)format.imageResolution.width * (double)format.imageResolution.height <=
