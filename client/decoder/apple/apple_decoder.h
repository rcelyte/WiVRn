...
namespace wivrn::apple {
	struct decoder {
		...
		wivrn::shard_accumulator *const accumulator;
		const vk::raii::SamplerYcbcrConversion ycbcr_conversion;
		...
		decoder(vk::raii::Device& device, vk::raii::PhysicalDevice &physical_device,
			const wivrn::to_headset::video_stream_description::item &description, float fps, uint8_t stream_index,
			std::weak_ptr<scenes::stream> scene, wivrn::shard_accumulator *accumulator);
		decoder(const decoder&) = delete;
		decoder(decoder&&) = delete;
		~decoder();
		void push_data(std::span<std::span<const uint8_t>> shards, uint64_t frame_index, bool partial);
		void frame_completed(const wivrn::from_headset::feedback &feedback,
