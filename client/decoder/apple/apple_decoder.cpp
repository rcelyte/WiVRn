...
#define ENABLE_YCBCR 0 // VK_KHR_sampler_ycbcr_conversion appears to broken in MoltenVK

wivrn::apple::decoder::decoder(vk::raii::Device &device, vk::raii::PhysicalDevice &physical_device,
		const wivrn::to_headset::video_stream_description::item &description, const float fps, const uint8_t stream_index,
		const std::weak_ptr<scenes::stream> scene, wivrn::shard_accumulator *const accumulator) :
		description(description), device(device), weak_scene(scene), accumulator(accumulator),
		ycbcr_conversion(device, {
			.format = vk::Format::eUndefined,
			.ycbcrModel = description.color_model ? vk::SamplerYcbcrModelConversion(*description.color_model) : vk::SamplerYcbcrModelConversion::eYcbcr709,
			.ycbcrRange = description.range ? vk::SamplerYcbcrRange(*description.range) : vk::SamplerYcbcrRange::eItuFull,
		...
		frameSampler(device, vk::StructureChain{
...
static void onDecodeFrame(void *const userptr, void*, const OSStatus status, const VTDecodeInfoFlags infoFlags,
	...
	if constexpr(ENABLE_YCBCR) {
		if(const size_t planeCount = CVPixelBufferGetPlaneCount(imageBuffer); planeCount != 2) {
			spdlog::error("expected 2 image planes, but got {}", planeCount);
			return;
		}
	} else {
	...
	vk::StructureChain imageInfo = {
		vk::ImageCreateInfo{
			...
			.format = ENABLE_YCBCR ? vk::Format::eG8B8R82Plane420Unorm : vk::Format::eR8G8B8A8Unorm,
			.extent = {(uint32_t)CVPixelBufferGetWidth(imageBuffer), (uint32_t)CVPixelBufferGetHeight(imageBuffer), 1},
			.mipLevels = 1,
			.arrayLayers = 1,
	...
	if constexpr(ENABLE_YCBCR)
		((CFTypeRef*)static_cast<VkImage>(*image))[32] = CFRetain(CVPixelBufferGetIOSurface(imageBuffer)); // HACK: bypass faulty validation in MoltenVK
	vk::StructureChain iv_info{
		vk::ImageViewCreateInfo{
			.image = *image,
			.viewType = vk::ImageViewType::e2D,
			.format = imageInfo.get<vk::ImageCreateInfo>().format,
			.components = ENABLE_YCBCR ? vk::ComponentMapping{} :
		...
		#if ENABLE_YCBCR
		vk::SamplerYcbcrConversionInfo{
			.conversion = *decoder->ycbcr_conversion,
		},
		#endif
	};
	vk::raii::ImageView imageView(decoder->device, iv_info.get());
...
void wivrn::apple::decoder::frame_completed(const wivrn::from_headset::feedback &feedback,
	...
	if(this->state.session != nullptr) {
		...
		// Available Pixel Formats - https://developer.apple.com/library/archive/qa/qa1501/_index.html
		const uint32_t format = ENABLE_YCBCR ? kCVPixelFormatType_420YpCbCr8BiPlanarFullRange : kCVPixelFormatType_32BGRA;

		const uint32_t size[2] = {this->description.video_width, this->description.video_height};
		const CFMutableDictionaryRef options = CFDictionaryCreateMutable(kCFAllocatorDefault, 4, &kCFTypeDictionaryKeyCallBacks, &kCFTypeDictionaryValueCallBacks);
...
std::vector<wivrn::video_codec> wivrn::apple::decoder::supported_codecs()
{
