...
xrt::drivers::wivrn::UDP::UDP(const bool ipv4)
{
...
static int recvmmsg(const int sockfd, struct mmsghdr *const msgvec, unsigned int, const unsigned int flags, struct timespec*) {
	const ssize_t length = recvmsg(sockfd, &msgvec->msg_hdr, flags);
	if(length == -1)
		return -1;
	msgvec->msg_len = length;
	return 1;
}
#endif
...
void xrt::drivers::wivrn::TCP::send_raw(const std::vector<std::span<uint8_t>> & spans)
{
	...
	msghdr hdr{
	        ...
	        .msg_namelen = 0,
	        .msg_iov = iovecs.data(),
	        .msg_iovlen = (decltype(hdr.msg_iovlen))(uint32_t)iovecs.size(),
	        .msg_control = nullptr,
	        .msg_controllen = 0,
	        .msg_flags = 0,
	};
